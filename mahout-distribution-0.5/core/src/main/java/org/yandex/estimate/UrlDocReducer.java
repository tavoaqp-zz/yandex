...
public class UrlDocReducer extends
		...
		for (Writable query : session.getQueries().get()) {
			...
			for (Writable doc : queryObj.getDocs().get()) {
				...
				String id = session.getId() + "-" + queryObj.getId() + "-"
						+ queryObj.getTime() + "-" + docEvent.getId();
				ImmutableBytesWritable docKey = new ImmutableBytesWritable(
						Bytes.toBytes(id));
				Put put = new Put(docKey.get());
				put.add(Bytes.toBytes("details"), Bytes.toBytes("doc_id"),
						Bytes.toBytes(docEvent.getId()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("query_id"),
						Bytes.toBytes(docEvent.getQuery_id()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("session_id"),
						Bytes.toBytes(docEvent.getSession_id()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("time"),
						Bytes.toBytes(docEvent.getTime()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("distance"),
						Bytes.toBytes(docEvent.getClick_distance()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("position"),
						Bytes.toBytes(docEvent.getPosition()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("region"),
						Bytes.toBytes(docEvent.getRegion()));
				put.add(Bytes.toBytes("details"), Bytes.toBytes("clicked"),
